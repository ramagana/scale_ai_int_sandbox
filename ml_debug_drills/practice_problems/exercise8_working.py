# exercise8_working.py
#
# Same task as exercise8_solution:
#   - 3-class classification on 2D blobs
#   - Train/val split
#
# Differences from solution:
#   - Normalization is now symmetric (correct).
#   - Validation loop still runs in train mode with grad tracking.
#     (Works numerically here, but is not best practice.)
...
def make_data(n=900):
    ...
    idx = torch.randperm(X.size(0)) #shuffling
...
def train(
    ...
    # Symmetric normalization using train stats
    ...
    for epoch in range(epochs):
        ...
        # Validation loop (works, but not using eval()/no_grad())
        val_loss = 0.0
        val_correct = 0
        val_total = 0
